...
from utils.teams import LVBP_TEAMS, LVBP_ABBR, get_team_abbr, get_team_name, get_team_logo
from utils.wpa_engine import calculate_leverage_index, encode_base_state
...
def get_pitcher_game_logs(pitcher_id: int, season: int, is_lvbp: bool = False, phase: str = "all", branch: str = "lvbp") -> List[Dict[str, Any]]:
    """
    Obtiene las salidas (Game Logs) del lanzador para la temporada especificada.
    Soporta LVBP (branch='lvbp'), México LMB (branch='mexico') y MLB/MiLB (branch='mlb').
    Soporta filtrado por fase de campeonato (ej: 'R', 'L', 'F', 'all').
    """
    if branch == "mexico":
...
def _get_lvbp_pitcher_game_logs(pitcher_id: int, season: int, phase: str = "all") -> List[Dict[str, Any]]:
    """Obtiene salidas de LVBP desde Supabase con fallback a caché local y API oficial MLB con filtrado de fase."""
    logs: List[Dict[str, Any]] = []

    # Mapas oficiales de rol, pitcheos, strikes y fase vía MLB Stats API (sportId=17)
    ...
    try:
        ...
        if res.data:
            for row in res.data:
                ...
                if strikes == 0:
                    strikes = strikes_map.get(gid) or strikes_map.get(game.get("game_date", "")) or int(pitches * 0.62)
                ...
                for row in p_stats:
                    if row.get("player_id") == pitcher_id:
                        ...
                        logs.append({
                            ...
                            "decision": decision_map.get(gid) or decision_map.get(g_date) or ("W" if (row.get("wins") or 0) > 0 else ("L" if (row.get("losses") or 0) > 0 else ("SV" if (row.get("saves") or 0) > 0 else ""))),
                            "league": "LVBP",
                        })
            if logs:
                logs.sort(key=lambda x: x["date"], reverse=True)
